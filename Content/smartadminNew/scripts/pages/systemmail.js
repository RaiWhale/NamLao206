...
//}
function handleRefresh() {
    // Thêm class refreshing để hiệu ứng
    emailList.classList.add('refreshing');
    ...
    // Lấy loại hiện tại từ hidden input
    const loai = document.getElementById('mail-context')?.value || '';

    // Lấy giá trị search nếu có
    const searchInput = document.querySelector('input[name="search"]');
    const search = searchInput ? searchInput.value : '';

    // Xây dựng URL
    let refreshUrl = '/TransportFiles/TransportFiles/HopThu?page=1';
    if (loai) {
        refreshUrl += '&loai=' + encodeURIComponent(loai);
    }
    if (search) {
        refreshUrl += '&search=' + encodeURIComponent(search);
    }
...
function handleDelete() {
    ...
    const threadIDs = Array.from(selectedEmails)
        .map(checkbox => {
            ...
            return id ? id.replace('msg-', '') : null;
        })
        .filter(id => id !== null)
        .map(id => parseInt(id));

    if (threadIDs.length === 0) {
        showToast('Không tìm thấy ID văn bản hợp lệ');
        return;
    }

    // Get context from hidden input
    const context = document.getElementById('mail-context')?.value || '';
    ...
    const deleteBtn = document.querySelector('.btn-icon.rounded-circle, .js-delete');
    if (deleteBtn) {
    ...
    // Send AJAX request
    fetch('/TransportFiles/TransportFiles/DeleteJqueryHopThu', {
        method: 'POST',
        headers: {
        ...
        .then(data => {
            if (data.success) {
                // Process each selected email with animation
                selectedEmails.forEach(checkbox => {
                    const listItem = checkbox.closest('li');
                    ...
                    setTimeout(() => {
                        ...
                        if (emailList.querySelectorAll('li').length === 0) {
                            // Show empty state
                            const emptyHtml = `
                        ...
                        `;
                            emailList.innerHTML = emptyHtml;
                        }
                    }, 300);
                });

                // Show success toast message
                showToast(`${count} message${count > 1 ? 's' : ''} deleted successfully`);

                // Update unread count if function exists
                if (typeof newEmailDisplayTab === 'function') {
                    newEmailDisplayTab();
                }

                // Uncheck select all if checked
                const selectAll = document.getElementById('js-msg-select-all');
                if (selectAll && selectAll.checked) {
                    selectAll.checked = false;
                    selectAll.indeterminate = false;
                }
            } else {
                console.error("Error processing emails: " + data.message);
        ...
        .catch(error => {
            ...
            showToast('Đã xảy ra lỗi khi kết nối đến server');
            // Reload on error to restore state
            setTimeout(() => location.reload(), 1000);
        })
        .finally(() => {
            ...
            // Remove tooltips if Bootstrap is loaded
