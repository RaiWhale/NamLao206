...
// Hàm khởi tạo nhiều bảng theo class (đặt ngoài DOM)
function initSmartTablesByClass(className = 'clientTable', baseOptions = {}) {
    const tables = document.querySelectorAll(`table.${className}`);

    if (tables.length === 0) {
        console.warn(`Không tìm thấy bảng nào với class "${className}"`);
        return [];
    }

    console.log(`Tìm thấy ${tables.length} bảng với class "${className}"`);

    const instances = []; // Lưu tất cả instance để trả về nếu cần

    tables.forEach((tableEl, index) => {
        if (!tableEl.id) {
            tableEl.id = `${className}-${index + 1}`;
        }

        const customOptions = { ...baseOptions };

        // Đọc data-attribute tùy chỉnh
        if (tableEl.dataset.pageLength) {
            customOptions.pageLength = parseInt(tableEl.dataset.pageLength, 10);
        }
        if (tableEl.dataset.searching === 'false') {
            customOptions.searching = false;
        }
        if (tableEl.dataset.paging === 'false') {
            customOptions.paging = false;
        }
        if (tableEl.dataset.responsive === 'false') {
            customOptions.responsive = false;
        }

        try {
            const instance = new SmartTables(tableEl.id, customOptions);
            instances.push(instance);
            console.log(`→ Khởi tạo thành công SmartTables cho #${tableEl.id}`);
        } catch (err) {
            console.error(`Lỗi khởi tạo SmartTables cho #${tableEl.id}:`, err);
        }
    });

    return instances; // Trả về mảng các instance (dễ quản lý nếu cần)
}
...
document.addEventListener('DOMContentLoaded', () => {
     ...
     * @type {SmartTables}
     */
    const clientTable = initSmartTablesByClass('clientTable', {
        /**
         * Data Configuration
         * -----------------
         ...
         */
        data: {
            //type: 'html',        // Data format is JSON
            //source: sampleData,  // Use the sample data array as the data source
            idField: 'data-id',       // Field that uniquely identifies each record
            columns: [
                // Cột Thao tác - được render bằng HTML
                {
                    data: null,
                    title: 'Thao tác',
                    className: 'never-hide',
                    orderable: false,
                    searchable: false,
                    render: (data, type, row) => {
                        // Lấy id linh hoạt (hỗ trợ cả Id và id)
                        const id = row.Id || row.id || '';
                        return `
                        <a href="#" class="edit-btn" data-id="${id}">Sửa</a>
                        <a href="#" class="delete-btn badge bg-warning" data-id="${id}">
        ...
        responsive: {
            enabled: true,                // bật responsive
            breakpoint: 992,              // ẩn cột khi màn hình < 992px (lg → md)
            // Quan trọng: bảo vệ cột có class never-hide
            protectedClasses: ['never-hide'],   // ← đây là phần bạn cần thêm (nếu thư viện hỗ trợ)
            // Nếu thư viện không có protectedClasses, dùng cách override bên dưới
        },
